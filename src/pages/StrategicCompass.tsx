...
const StrategicCompass: React.FC = () => {
    ...
    const equityAnalysis = useMemo(() => {
        ...
        // 🔥 SSOT: stats.inputs.cash를 사용하여 financials 의존성 완전히 제거
        const liquidOffset = (stats?.liquidCash?.value || 0) - (stats?.inputs?.cash || 0);
        let fundingIndex = -1;
        ...
        for (let i = nowIndex; i < ssotMetrics.cashflow.length; i++) {
            ...
            const remLiquidRunway = stats?.survivalRunway || 0;
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                        ...
                        <div className="flex flex-wrap items-center gap-10">
                            <ExplainableKPI
                                label="🔥 Burn (시뮬레이션)" 
                                result={{ 
                                    value: Math.abs(stats?.netBurn || 0),
                                    dataSource: 'scenario',
                                    formula: 'Net Burn = Cash Out - Cash In',
                                    period: 'Recent',
                                    inputs: stats?.inputs
                                }} 
                                description="엔진 기반 순 소진액"
                                color="text-rose-500"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <ExplainableKPI 
                        label="💸 Runway (시뮬레이션)" 
                        result={{ 
                            value: stats?.strategicRunway || 0,
                            dataSource: 'scenario',
                            formula: 'Runway = Cash / Net Burn',
                            period: 'Projection',
                            inputs: stats?.inputs
                        }} 
                        description="엔진 기반 런웨이"
                        color="text-indigo-400" 
                        formatValue={(v) => (
                            <span className="text-xl font-black italic">
                                {(v === Infinity || v > 60) ? "지속 가능" : `${v.toFixed(1)}개월`}
                            </span>
                        )} 
                    />
                    <ExplainableKPI 
                        label="📈 Strategic Runway" 
                        result={{
                            value: stats?.runway?.strategic || 0,
                            dataSource: 'scenario',
                            formula: 'Simulated Cash Out Point',
                            period: '36-Month View',
                            inputs: stats?.inputs
                        }} 
                        color={strategicRunway >= 12 ? 'text-emerald-400' : 'text-rose-400'} 
                        formatValue={(v) => <span className="text-xl font-black italic">{(v === null || v === undefined) ? "지속 가능" : (v === Infinity ? "∞" : `${v.toFixed(1)}개월`)}</span>} 
                    />
                    <ExplainableKPI 
                        label="흑자 전환 시점" 
                        result={{ 
                            value: breakEvenMonth || 0, 
                            formula: 'Scenario Net Income > 0 Check', 
                            period: 'Simulation', 
                            dataSource: 'scenario',
                            inputs: stats?.inputs
                        } as any} color="text-indigo-400" formatValue={(v) => <span className="text-xl font-black italic">{breakEvenMonth != null ? `${breakEvenMonth}개월` : "N/A"}</span>} 
                    />
                    <ExplainableKPI 
                        label="Gross Burn" 
                        result={{ 
                            value: grossBurn, 
                            formula: 'Avg Cash Outflow', 
                            period: 'Recent 6m', 
                            dataSource: 'scenario',
                            inputs: stats?.inputs
                        } as any} color="text-slate-400" formatValue={(v) => <span className="text-xl font-black italic">{formatCurrency(v)}</span>} 
                    />
                </div>
